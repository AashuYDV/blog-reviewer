...
def run_initial_review(api_key: str, blog_text: str, fact_check_text: str = "") -> str:
    ...
    prompt = (
        ...
        "BEFORE YOU BEGIN THE FULL REVIEW — MANDATORY SOURCE AUDIT (do this first):\n"
        "Read through the entire blog and find every stat, number, fee, ranking, deadline, "
        "policy rule, salary figure, or factual claim.\n"
        "For each one, apply this exact rule:\n"
        "  -> If it is immediately followed by [LINK: url] in the text: it is already sourced. "
        "Do NOT flag it. Move on.\n"
        "  -> If it has NO [LINK: url] directly after it AND no named official source inline: "
        "it is unsourced. You MUST flag it in the section-wise review as:\n"
        "     Issue: No source for '[exact claim]'.\n"
        "        Fix: Add [SOURCE NEEDED: suggested official source]\n"
        "Every unsourced stat must be flagged individually — do not group them, do not skip any.\n"
        "This source audit is non-negotiable and must be completed before anything else.\n\n"
    ...
    <div class="hero-wrap">
        <div class="hero-logo">
            <img src="https://d14lg9nzq1d3lc.cloudfront.net/advance-website/assets/images/company-logo/logo.svg"
                 style="width:230px;height:55px;object-fit:contain;filter:brightness(0) invert(1);" alt="Leap Finance" />
        </div>
